...
impl EntryData {
  ...
  pub(crate) fn create_from_metadata(metadata: std::io::Result<Metadata>, name: impl Into<String>) -> Result<Self, Error> {
    let metadata = metadata?;
    let size = metadata.len();
    let modify = metadata.modified().unwrap_or(SystemTime::UNIX_EPOCH).elapsed().unwrap().as_nanos();
