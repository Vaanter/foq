...
#[tracing_attributes::instrument(skip(command_processor, reply_sender))]
pub(crate) async fn dele(
  command: &Command,
  command_processor: &mut CommandProcessor,
  reply_sender: &mut impl ReplySend,
) {
  assert_eq!(Commands::Dele, command.command);
...
mod tests {
  ...
  async fn dele_absolute_test() {
    ...
    let command = Command::new(Commands::Dele, format!("/{}/{}", label, file_name));
  ...
  async fn dele_relative_test() {
    ...
    touch(&file_path).expect("Test file must exist");
    let _cleanup = FileCleanup::new(&file_path);
    let command = Command::new(Commands::Dele, &file_name);
  ...
  async fn dele_folder_test() {
    ...
    let command = Command::new(Commands::Dele, &dir_name);
