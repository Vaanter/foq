...
pub(crate) async fn rmd(
  ...
  let result = session_properties.file_system_view_root.delete_folder(&command.argument).await;

  reply_sender.send_control_message(get_delete_reply(result, true)).await;
}
