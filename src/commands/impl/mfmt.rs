...
pub(crate) async fn mfmt(
  command: &Command,
  command_processor: Arc<CommandProcessor>,
  reply_sender: Arc<impl ReplySend>,
) {
  debug_assert_eq!(command.command, Commands::Mfmt);
...
mod tests {
  ...
  #[tokio::test]
  async fn mfmt_not_logged_in_test() {
    let command = Command::new(Commands::Mfmt, "20020717210715 file");
  ...
  #[tokio::test]
  async fn mfmt_invalid_timeval_test() {
    let command = Command::new(Commands::Mfmt, "INVALID file");
  ...
  #[tokio::test]
  async fn mfmt_relative_with_label_test() {
    let label = "test";
    let root = temp_dir();
    let file_name = format!("{}.test", Uuid::new_v4().as_hyphenated());
    let file_path = root.join(&file_name);
    ...
    let timeval = Local::now().sub(TimeDelta::hours(4)).with_nanosecond(0u32).unwrap();
    let command =
      Command::new(Commands::Mfmt, format!("{} {}/{}", format_timeval(&timeval), label, file_name));
  ...
  async fn mfmt_relative_test() {
    ...
    let timeval = Local::now().sub(TimeDelta::hours(4)).with_nanosecond(0u32).unwrap();
    let command =
      Command::new(Commands::Mfmt, format!("{} {}", format_timeval(&timeval), &file_name));
